...
DATABASE = rf"B:\Computer Science and Engineering\PycharmIDE\ManipalAttendanceProjectV2\Database\database.db"
...
def is_point_in_polygon(x_provided, y_provided, polygon_coords):
    x, y = x_provided, y_provided
    inside = False
...
def check_location(email, user_x, user_y):
    # load the email and class from user_details in .db

    table_name = "user_details"
    first_conn = sqlite3.connect(DATABASE)
    cursor = first_conn.cursor()
    cursor.execute(f"SELECT email, class FROM {table_name} WHERE email = ?", (email,))
    result = cursor.fetchone()
    if result is None:
        return None
    else:
        email_, class_ = result
        print(email_, class_)

        first_conn.commit()
        first_conn.close()

        # load the coordinates from map_class_to_coordinates in .db

        second_conn = sqlite3.connect(DATABASE)
        second_cursor = second_conn.cursor()
        second_cursor.execute(f"SELECT X1, Y1, X2, Y2, X3, Y3, X4, Y4 FROM map_class_to_coordinates WHERE class = ?", (class_,))
        ...
        if result is None:
            ...
            x_provided = float(user_x)
            y_provided = float(user_y)

            return is_point_in_polygon(x_provided, y_provided, polygon_coords)


@app.route('/submit_location', methods=['POST'])
def submit_location():
    print("submit_location: function is called")
    email = request.form.get('username')
    user_x = request.form.get('user-x')
    user_y = request.form.get('user-y')

    print(email, user_x, user_y)

    # Check if the user's location is within the desired region
    if check_location(email, user_x, user_y):
        # Call another function if the location is within the region
        # your_function_to_call_when_location_is_within_region()
        print("User is in the desired location.")
        return "Location is within the desired region."
    else:
        print("User is not in the desired location.")
        return "User not in the desired location."
